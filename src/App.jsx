...
export default function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const bookingsRes = await fetch('/api/get_bookings.php');
        const loadedBookings = await bookingsRes.json();
        // Защита: если API вернуло ошибку или HTML, устанавливаем пустой массив
        if (Array.isArray(loadedBookings)) {
            setBookingsList(loadedBookings);
        }

        // --- ЗАГРУЗКА КОНТЕНТА ИЗ FIREBASE ---
        const contentSnap = await getDocs(collection(db, "site_content"));
        contentSnap.docs.forEach(doc => {
          if (doc.id === 'hero') setHeroData(doc.data() || defaultHero);
          if (doc.id === 'seo') setSeoData(doc.data() || defaultSeo);
  ...
  const handleBookingSubmit = async (e) => {
    ...
    // Форматируем дату так, чтобы база MySQL ее 100% поняла:
    const sqlFormatDate = new Date().toISOString().slice(0, 19).replace('T', ' ');
    ...
    const newBooking = {
      ...
      endTime: endTime,
      status: 'new',
      createdAt: sqlFormatDate
    };

    const WEBHOOK_URL = 'ВАШ_WEBHOOK_URL_ЗДЕСЬ'; 

    try {
      // --- ОТПРАВКА В НОВУЮ БАЗУ MYSQL ---
      const res = await fetch('/api/add_booking.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(newBooking)
      ...
      console.error("Ошибка при сохранении заявки:", error);
      alert("Не удалось отправить заявку. Проверьте настройки подключения к БД.");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleImageUpload = (e) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        if (contentTab === 'hero') setEditingItem({ ...editingItem, bgImage: reader.result });
        else setEditingItem({ ...editingItem, image: reader.result });
      };
      reader.readAsDataURL(file);
    }
  };
